...
function validateCSVFile(file) {
  if (!file || !file.name || !file.name.toLowerCase().endsWith(".csv")) return "invalid_type";
  if (file.size > CSV_LIMITS.maxBytes) return "too_large";
  return null;
}
...
function readCSVFile(file, onSuccess, onError) {
  ...
  reader.onerror = () => onError("read_error");
  reader.readAsText(file);
}

function parseCSV(text) {
  if (typeof text !== "string" || text.length > CSV_LIMITS.maxBytes) {
    return { headers: [], rows: [], error: "too_large" };
  }
  ...
  if (!rows.length) return { headers: [], rows: [], error: null };
